...
import { ArrowLeft, ArrowRight, Hamburger } from 'lucide-react'
...
export default function Page() {
	...
	const { data, error, isLoading } = useSWR(`/api/manage/menus?page=${page}&limit=${limit}`, fetcher, {
		revalidateOnMount: true,
		revalidateOnFocus: true,
	})

	const listMenus = data?.menu?.data
			...
			<div className='min-h-screen max-w-screen w-full h-full p-4 overflow-hidden border space-y-4 '>
				<div className="w-full flex justify-between border rounded-lg p-4 shadow-lg">
					<div className="flex justify-center items-center gap-2">
						<Hamburger className='size-8 text-primary' />
						<h3 className="font-bold text-3xl text-primary">Manage Menus</h3>
					</div>
				...
				<div className="rounded-lg border w-full h-full shadow-lg">
					<DataTable columns={columns} data={listMenus?.menus} />
					<div className="flex justify-end items-center gap-4 py-4 p-2">
						...
						<Button
							variant="outline"
							onClick={nextPage}
							disabled={page >= listMenus?.meta?.totalPages}
						>
							Next
